...
- **验证证据：** 指定核心二进制路径存在；发现已有 `easytier-core` 进程（PID `2908`）。该进程的启动时间与模块路径因权限限制不可读；未读取命令行，从而不会接触或记录密钥。
- **风险/阻塞：** 无法确认现有进程是否已使用指定测试配置。直接再启动节点可能在同一虚拟地址上冲突；停止现有 PID 属于影响现有网络连接的操作，尚未获得明确授权。启动 TUN 设备还需要管理员权限且可能受驱动或防火墙影响；启动成功不替代 Linux CI 的端到端验收。
- **下一步：** 部署者确认 PID `2908` 可复用，或明确授权停止该 PID 后再按指定参数启动新的隐藏后台节点。
