...
- **风险/阻塞：** 服务器端完成不等于 GUI 流程完成。受信 HTTPS:8443 未可用前，不能执行或宣称 GUI 邀请码兑换、DPAPI 保存和真实数据面启动已验收。真实密钥、管理员令牌和邀请码未写入仓库、日志或验证报告。
- **下一步：** 部署者确认 HTTPS:8443 可访问后，创建一次性邀请码，执行 Windows GUI 兑换→DPAPI 保存→守护进程启动→真实对端连通的端到端验收；通过后将本任务标为 PASS。
