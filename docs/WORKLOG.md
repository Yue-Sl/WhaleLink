...
### WL-0072 — Computer Use 运行时存在但初始化失败
...
- **关键命令：** 运行时目录/manifest/二进制存在性检查；Computer Use `getState()` 初始化重试；部署者重启 Codex 后的第二次 `getState()` 复测；非敏感 `codex login status`；官方设备授权与本机 OAuth 回调登录尝试。
- **验证证据：** [`docs/verification/2026-09-19-computer-use-runtime-check.md`](verification/2026-09-19-computer-use-runtime-check.md) 与 [`docs/verification/2026-09-19-computer-use-auth-blocked.md`](verification/2026-09-19-computer-use-auth-blocked.md)。运行时文件存在；两次初始化均返回 `unsupported Codex auth method: apikey`，应用枚举为空。诊断确认现有会话为 API key 会话；设备授权和 OAuth 回调登录均在浏览器端返回 `invalid_state`，已安全取消。
- **风险/阻塞：** 不能用猜测坐标、终端 UI 自动化或直接 API 调用冒充 GUI 验收；本次 GUI 流程未执行。服务器端控制面和 HTTPS 入口仍可用。
- **下一步：** 由 Codex/OpenAI 账户或桌面端维护方修复账户登录的 `invalid_state`，使 Codex 获得 ChatGPT 账户会话；恢复后使用新的短期邀请码继续验收。
...
- **状态：** PASS
- **目标与前置条件：** 部署者需要一份不依赖分散工作日志即可查看的工程计划、实际进度、剩余事项和当前精简版使用说明；不得把任何部署环境参数写入文档。
- **改动：** 新增 [`docs/PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)，以 M0--M4 对照表、状态图例、首版能力边界、连接步骤和按依赖排序的剩余工作集中呈现当前状态。
- **关键命令：** 只读复核工程规格、工作日志、使用说明、发行检查清单与现有非敏感验证报告；`git diff --check`。
- **验证证据：** 对照内容仅引用既有的发行、本机冒烟及跨机验收报告；文档示例使用占位参数，未包含真实 Relay、网络名、地址、凭据或令牌。格式检查通过。
- **风险/阻塞：** 此文档是状态快照，不替代 `docs/WORKLOG.md` 的逐任务事实记录或 `docs/verification/release-checklist.md` 的最终发布门禁；M2 部署、GUI 真实兑换、Linux/CI/GitHub Release 仍未完成。
- **下一步：** 先完成 M2 HTTPS 控制面部署与 GUI 邀请码端到端验收，再依次完成 Linux、CI 和 GitHub Release 门禁。
...
### WL-0069 — 发行包跨机 TUN 验收归档与固定 RPC 冲突修复

- **状态：** PASS
- **目标与前置条件：** 部署者提供了发行包跨机 TUN 数据面 PASS 记录，并报告 `--no-listener` 场景出现 Windows 10048；需区分上游数据监听、上游管理端口与 WhaleLink 参数转发行为。
- **改动：** 将 `whalelinkd connect` 的 `--rpc-portal` 改为可选参数；用户未显式指定时不再传递该参数，避免强制复用固定本机 RPC 端口。`--no-listener` 保持原样转发。
- **关键命令：** 上游帮助检查；专属子进程监听端口检查；两个随机隔离实例的固定 RPC 冲突复现；修复后双实例并存验证。
- **验证证据：** [`docs/verification/2026-09-19-cross-machine-acceptance.md`](verification/2026-09-19-cross-machine-acceptance.md)。部署者的跨机 TUN 数据面验收 PASS；固定 RPC 复用确定性触发 Windows 10048；修复后两实例均存活、拥有不同的上游自动管理端口、且不监听 11010。
- **风险/阻塞：** 上游仍会创建自动分配的本机管理端口；这不等于数据监听且不会与并行 WhaleLink 实例冲突。用户若显式设置 `--rpc-portal`，仍须自行保证端口唯一。
- **下一步：** 重建 Windows 发行物并更新桌面交付目录；M2 控制面部署后执行 GUI 邀请码端到端验收。

### WL-0066 — 首版用户交付目录与使用说明

- **状态：** PASS
- **目标与前置条件：** 部署者要求确认首版可用性，并将完整发行包与中文使用说明交付至本机桌面。
- **改动：** 新增首版中文使用说明，覆盖参数获取、校验、管理员连接、断开、GUI 边界与常见故障；Windows 打包流程将说明放入便携包。已重建发行物并复制到桌面交付目录。
- **关键命令：** `scripts/package-windows.ps1`；SHA-256/发行物内容检查；受限外的桌面目录复制。
- **验证证据：** Windows 打包通过，便携包内存在 `docs/WhaleLink-User-Guide.md`；桌面新建交付目录包含便携包、安装包、`SHA256SUMS.txt` 和中文说明。便携包、安装包与校验和桌面副本的 SHA-256 均与构建产物一致。
- **风险/阻塞：** 真实 TUN 仍要求安装机管理员权限；GUI 邀请兑换需要另行部署 HTTPS 控制面，首版直连路径不依赖它。
- **下一步：** 使用安装机管理员权限按说明输入实际部署参数，执行首次对端业务连通验收。
...
- **状态：** PASS
- **目标与前置条件：** 部署者要求建立本地 AI 与服务器 AI 的持久对接文件，避免在诊断证据不足时重复执行 CI 或修改配置。
- **改动：** 新增 `docs/SERVER_AI_HANDOFF.md`。该文件规定服务器/CI/真实网络相关操作前必须读取，记录非敏感事实、待回答问题、服务器 AI 回复和获批的下一步。
- **关键命令：** 创建对接文件；`git diff --check`；推送供服务器 AI 读取。
- **验证证据：** 当前 Docker 超时、TUN 未验收和多实例本机 RPC 限制已浓缩为三个可回答的非敏感诊断问题；文件禁止保存真实配置、凭据和原始日志。
- **风险/阻塞：** 在服务器 AI 回复前，本地 AI 不得重跑 CI 或调整服务器、Docker、EasyTier 配置；这是一项协作门槛，不替代实际发布验收。
- **下一步：** 服务器 AI 写入非敏感诊断结论；本地 AI 先读取并确认单一修复方向，再执行最小变更。
...
- **状态：** PASS
- **目标与前置条件：** 部署者已废弃先前隔离测试的虚拟网段与 PC/CI 节点地址，并提供新网段和对应节点地址；Relay、网络名与密钥保持既有受保护配置。
- **改动：** 无源码硬编码改动。Linux 工作流已通过 `WHALELINK_TEST_IPV4` 与 `WHALELINK_TEST_PEER` 注入地址，脚本在运行时生成临时 `0600` TOML，因此地址轮换不需要改变 Git 内容。
- **关键命令：** 扫描工作树，确认旧/新隔离网段、节点地址、Relay、网络名与密钥均未进入追踪内容。
- **验证证据：** 静态扫描未发现部署者提供的旧或新测试值。工作流的 `WHALELINK_TEST_IPV4` 为可选固定地址，`WHALELINK_TEST_PEER` 为对端地址；二者均是仓库 Actions Variable，不是源码常量。
- **风险/阻塞：** 仓库管理员仍须在 GitHub Actions Variables 更新新地址值；本机已有的 PID `2908` 进程冲突仍使 WL-0059 保持 BLOCKED，未获得停止授权前不能安全启动新 PC 对端。
- **下一步：** 管理员更新受保护 Variables 后确认；如需本机对端，确认复用 PID `2908` 或授权停止后按新地址启动。

### WL-0059 — Windows 外部对端测试节点启动

- **状态：** PASS
- **目标与前置条件：** 部署者授权在本机以管理员权限后台启动一个隔离测试网络的 EasyTier Windows 节点，作为 Linux CI 真实 TUN 验收的外部对端。
- **改动：** 待核验指定 EasyTier 可执行文件路径、检查是否已有同名核心进程，并以隐藏后台进程启动；仅记录进程存活与 PID，不记录网络名、密钥、Relay、虚拟地址或命令行。
- **关键命令：** `Test-Path`；按进程名读取现有 PID；`Start-Process -WindowStyle Hidden`；按 PID 复核存活。
- **验证证据：** 指定核心二进制路径存在；启动请求的初始 PID 为 `43420`，该启动器随后退出。通过不读取命令行的进程谱系与五秒存活检查，确认两个 `easytier-core` 进程仍在运行：服务管理器托管的现网节点 PID `58924`，以及由本次 PowerShell 启动链路托管的独立测试节点 PID `62300`。在启动后检查中，旧 PID `2908` 已不再存在；本任务未对其执行停止或重启操作。
- **风险/阻塞：** 测试节点的进程存活不等于已完成 Relay 加入、虚拟地址分配或服务器端成员可见性；这些需要服务器侧 CLI 或 Linux CI 的独立验证。PID 可能在服务/进程自身重启后变化，后续诊断必须按角色与当前时间重新确认，不应依赖旧 PID。
- **下一步：** 由服务器管理员可选验证隔离测试成员可见性；更新 GitHub Actions 的新网段 Variables 后重跑 Linux 真实 TUN 门禁。
