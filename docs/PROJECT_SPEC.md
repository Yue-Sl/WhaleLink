...
Windows 客户端接受原始邀请码，或由受控渠道生成的 `whalelink://invite` 二维码解码载荷；
该载荷必须包含 HTTPS 控制面地址和一次性邀请码，客户端只在内存中解析，随后仍要求用户主动兑换。
...
`whalelinkd` 在本地 IPC 的健康/状态/诊断查询中回收异常退出的数据面子进程，并按最后一次
明确启动的房间尝试受控重启；用户显式停止会取消该重启意图。重启尝试节流且不记录入网材料。

## 3. 身份、房间与数据

- 管理员令牌只用于管理 API，使用 `X-WhaleLink-Token` 传输；不得记录、导出或显示。
- 邀请码为高熵、短期、单次、可撤销的服务器端凭据；兑换后立即失效。
- 客户端持久化的入网材料在 Windows 上必须由 DPAPI 保护；桌面端以当前用户 SID 显式设置受保护 DACL，daemon 以 Owner Rights DACL 保护当前用户创建的凭据目录。
